...
module.exports = function (gulp) {
  return function () {
    return glob(`${config.directories.projectDirectory}**/salesforce.scss`, (err, files) => {
      const tasks = files.map((entry) => {
        const themeName = utils.getThemeName(entry);

        return gulp
          .src([`${config.directories.featureDirectory}**/salesforce.scss`, entry])
          .pipe(sass(scssMixinsOptions).on('error', sass.logError))
          .pipe(concat(config.bundle.cssBundleNameSalesForce))
          .pipe(postcss([
            autoprefixer(),
            cssnano({
              zindex: false
